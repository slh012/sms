...
*/
	...
	class getPremiumSMSInbound extends textAnywhere{
		
		/*
		 * Retrieve inbound messages sent to a short code/keyword service
		 * http://developer.textapp.net/WebService/Methods_GetPremiumSMSInbound.aspx
		 */
		...
		protected function xmlresponse(){
			...
			$xPathResult = $this->xpath->search("//PSMSInbounds/InboundPSMS");
			if(!empty($xPathResult)){
				for($i=0; $i < $xPathResult->length; $i++){
					...
					$this->result[$id]['RBID'] = $this->xpath->search("//RBID", $xPathResult->item($i))->item($i)->nodeValue;
